"""
https://github.com/maheshpaulj/Lane_Detection/blob/main/video.py

Deprecated
Not using at the moment because of accuracy. 
Instead using the static ego-zone. 
"""
...
def draw_lane_lines(img, left_line, right_line, color=[0, 255, 0], thickness=10):
    """draw the filled polygon between the lane lines"""
...
def pipeline(image):
    ...
    min_y = int(image.shape[0] * 0.75)  # Top edge of the green polygon. Larger = shorter polygon.
...
def process_video():
    ...
    # Open the video file
    cap = cv2.VideoCapture('../data/driving-sample-2.mp4')
