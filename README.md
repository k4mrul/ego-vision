...
This project performs real-time driving action prediction from car dashcam video by combining YOLOv11 for object detection, ByteTrack for multi-object tracking, and Depth Anything V2 for depth estimation. Based on the detected results, the system classifies each scene into one of three driving actions: GO, SLOW DOWN, or STOP. The predicted action is rendered as a live overlay on the video.
...
The decision is based on a single question: is any tracked object inside the
ego-zone trapezoid close enough to require an action?

| Order | Action | When |
|---|---|---|
| 1 | **STOP** | Any object in the path ahead is within `LEAD_STOP_DISTANCE_M` |
| 2 | **SLOW DOWN** | Any object in the path ahead is within `LEAD_YIELD_DISTANCE_M` |
| 3 | **GO** | Path is clear or objects are far enough away |

To stop the on-screen action from flickering, a new action has to repeat for n
number of frames in a row before it actually changes on screen.
...
|---|---|---|
| `LEAD_STOP_DISTANCE_M` | `10.0` | An object this close in your path makes you stop |
| `LEAD_YIELD_DISTANCE_M` | `40.0` | An object this close in your path makes you slow down |
...
│   └── inference_resizer.py downscales frames for faster inference
├── reasoning/
...
- [x] Simplify to distance-only decision (removed traffic-light classification, TTC, lane detection)
- [ ] Integrate a lane-detection model so the path-ahead shape follows the real lane instead of a fixed guess
- [ ] Improve speed estimation in the HUD. The code needs more accurate distance readings first.
